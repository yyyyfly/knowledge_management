<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    ...
    <div class="mb-8">
      <div class="flex items-center space-x-4">
        <div class="w-12 h-12 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div>
          <h1 class="text-3xl font-bold text-gray-900">技能笔记</h1>
          <p class="text-gray-600">技能学习和实践效果记录</p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        <div v-for="note in recommendedNotes" :key="note.id"
             ...
             class="bg-white rounded-xl p-6 border-2 border-gray-200 hover:border-purple-400 hover:shadow-lg cursor-pointer transition-all">
      ...
      <div v-if="recommendedNotes.length === 0" class="text-center py-12">
        <i class="fas fa-check-circle text-5xl text-purple-500 mb-4"></i>
    ...
    <div @click="showNotesList = true"
         class="bg-gradient-to-br from-purple-50 to-indigo-50 border-2 border-purple-200 rounded-2xl p-8 hover:shadow-lg cursor-pointer transition-all text-center">
        ...
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
          ...
          <div class="flex items-center justify-between p-6 border-b border-gray-200">
            <div class="flex items-center space-x-3">
              ...
              <div>
                <h3 class="text-2xl font-semibold text-gray-900">技能笔记列表</h3>
                <p class="text-sm text-gray-500">{{ skillNotes.length }} 条笔记</p>
              </div>
            </div>
            <button 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                ...
                <div class="flex flex-wrap gap-1">
                    <span v-if="note.skillType" class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-medium">
                      {{ note.skillType }}
                  </span>
                </div>
              </div>
      ...
      <Transition name="modal-slide">
        <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
          <div class="flex items-center justify-between mb-6">
              ...
              <div class="bg-purple-50 rounded-xl p-4 mb-4">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <span class="text-lg font-bold text-purple-600 ml-2">已巩固 {{ selectedNote.reviewCount || 0 }} 次</span>
                  </div>
            ...
            <!-- 三个富文本字段 -->
            <div>
              <p class="text-sm font-medium text-gray-700 mb-2">预期描述：</p>
                <div class="note-content bg-gray-50 p-4 rounded-lg min-h-[100px]" v-html="selectedNote?.expectedDescription || '暂无内容'">
                </div>
            </div>
            
            <div>
              <p class="text-sm font-medium text-gray-700 mb-2">思考总结：</p>
                <div class="note-content bg-gray-50 p-4 rounded-lg min-h-[100px]" v-html="selectedNote?.thinkingSummary || '暂无内容'">
                </div>
            </div>
            
            <div>
              <p class="text-sm font-medium text-gray-700 mb-2">最终效果：</p>
                <div class="note-content bg-gray-50 p-4 rounded-lg min-h-[100px]" v-html="selectedNote?.finalEffect || '暂无内容'">
                </div>
            </div>
            ...
            <div class="flex items-center space-x-4 text-sm text-gray-500">
              ...
              <div class="flex gap-4 mt-6">
                <button @click="handleCompleteReview" 
                        class="flex-1 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white py-4 rounded-xl font-medium transition-all duration-300 hover:shadow-lg">
...
// 响应式数据
const skillNotesData = ref<SkillNote[]>([])
const recommendedNotes = ref<SkillNote[]>([])

// 加载技能笔记
const loadNotes = async () => {
  try {
    const notes = await getNotesByType('skill')
    skillNotesData.value = notes.map((note: any) => ({
      ...note,
      reviewCount: note.reviewCount || 0,
      skillType: note.skillType || '未分类'
    })) as SkillNote[]
  } catch (error) {
    console.error('加载技能笔记失败:', error)
  }
}
...
const loadRecommendedNotes = async () => {
  try {
    const response = await getRecommendedNotes('skill')
...
const skillNotes = computed(() => skillNotesData.value)

const showNotesList = ref(false)
const showNoteDetail = ref(false)
const selectedNote = ref<SkillNote | null>(null)
const searchQuery = ref('')

const filteredNotes = computed(() => {
  let notes = skillNotes.value
...
const viewNoteDetail = (note: SkillNote) => {
