  ...
  <section id="diplomatic-projects-dashboard" class="p-6 animate-fade-in">
    ...
    <div class="mb-8">
      <div class="flex items-center space-x-4 mb-4">
        ...
        <span class="bg-green-100 text-green-600 px-3 py-1 rounded-full text-sm font-medium">人脉拓展</span>
      </div>
      <p class="text-gray-600">人脉经营、社交网络、合作资源的长期建设中心，聚焦维护重要关系、拓展优质人脉、实现资源互换</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-soft p-6 mb-8">
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center space-x-2">
          ...
          <select v-model="statusFilter" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
...
const loadProjectInsights = async () => {
  try {
    ...
    if (response.code === 200) {
      ...
      console.log('【外交行动】加载项目心得数据:', projectRecords.value.length, '个')
    }
  } catch (error) {
    console.error('【外交行动】加载项目心得失败:', error)
  }
}
...
const loadIssues = async () => {
  try {
    ...
    if (response.code === 200) {
      ...
      console.log('【外交行动】加载问题记录数据:', issues.value.length, '个')
    }
  } catch (error) {
    console.error('【外交行动】加载问题记录失败:', error)
  }
}
...
const filteredProjects = computed(() => {
  if (!statusFilter.value) return diplomaticProjects.value
  return diplomaticProjects.value.filter(p => p.status === statusFilter.value)
})
