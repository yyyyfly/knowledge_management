  ...
  <section id="construction-projects-dashboard" class="p-6 animate-fade-in">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      ...
      <div class="bg-white rounded-xl shadow-soft p-6">
        ...
        <p class="text-2xl font-bold text-blue-600">{{ constructionProjects.filter(p => p.status === 'active').length }} 个</p>
...
const filteredProjects = computed(() => {
  if (!statusFilter.value) return constructionProjects.value
  return constructionProjects.value.filter(p => p.status === statusFilter.value)
})
