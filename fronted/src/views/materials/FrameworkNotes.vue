    ...
    <div class="mb-8">
      <div class="flex items-center space-x-4">
        <div class="w-12 h-12 bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl flex items-center justify-center shadow-lg">
          ...
          <div>
          <h1 class="text-3xl font-bold text-gray-900">框架笔记</h1>
          <p class="text-gray-600">系统性知识框架和体系构建</p>
        </div>
      </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                ...
                <div class="flex flex-wrap gap-1">
                    <span v-if="note.subjectType" class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-medium">
                      {{ note.subjectType }}
                  </span>
                </div>
              </div>
...
// 响应式数据
const frameworkNotesData = ref<FrameworkNote[]>([])
const recommendedNotes = ref<FrameworkNote[]>([])

// 加载框架笔记
const loadNotes = async () => {
  try {
    const notes = await getNotesByType('framework')
    frameworkNotesData.value = notes.map((note: any) => ({
      ...note,
      reviewCount: note.reviewCount || 0,
      subjectType: note.subjectType || '未分类'
    })) as FrameworkNote[]
  } catch (error) {
...
const loadRecommendedNotes = async () => {
  try {
    const response = await getRecommendedNotes('framework')
...
const frameworkNotes = computed(() => frameworkNotesData.value)

const showNotesList = ref(false)
const showNoteDetail = ref(false)
const selectedNote = ref<FrameworkNote | null>(null)
const searchQuery = ref('')

const filteredNotes = computed(() => {
  let notes = frameworkNotes.value
...
const viewNoteDetail = (note: FrameworkNote) => {
