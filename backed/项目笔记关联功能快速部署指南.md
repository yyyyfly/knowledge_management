# 项目笔记关联功能 - 快速部署指南

## 🚀 一键部署（推荐）

### 步骤1：数据库部署

```bash
# 1. 连接到MySQL数据库
mysql -u root -p knowledge_management

# 2. 执行建表脚本
source backed/create_project_note_relation_table.sql

# 3. 验证表创建成功
DESC project_note_relation;
```

**预期输出：**
```
+------------------+--------------+------+-----+-------------------+
| Field            | Type         | Null | Key | Default           |
+------------------+--------------+------+-----+-------------------+
| id               | int          | NO   | PRI | NULL              |
| project_id       | int          | NO   | MUL | NULL              |
| note_id          | int          | NO   |     | NULL              |
| note_type        | varchar(50)  | NO   | MUL | NULL              |
| rec_creator      | varchar(50)  | YES  |     | NULL              |
| rec_create_time  | datetime     | YES  |     | CURRENT_TIMESTAMP |
| rec_revisor      | varchar(50)  | YES  |     | NULL              |
| rec_revise_time  | datetime     | YES  |     | CURRENT_TIMESTAMP |
| arch_flag        | char(1)      | YES  |     | 1                 |
+------------------+--------------+------+-----+-------------------+
```

### 步骤2：后端部署

```bash
# 进入后端目录
cd backed

# 清理并重新编译
mvn clean package -DskipTests

# 停止旧服务（如果正在运行）
# ./stop.sh 或手动停止

# 启动新服务
java -jar target/knowledge-management-system-1.0.0.jar
```

### 步骤3：验证部署

```bash
# 测试1：获取项目1的关联笔记（应返回空数组）
curl http://localhost:8080/project/1/notes

# 测试2：添加笔记到项目
curl -X POST http://localhost:8080/project/1/notes \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{"noteId": 1, "noteType": "framework"}'

# 测试3：再次获取（应返回[1]）
curl http://localhost:8080/project/1/notes
```

## ✅ 部署验证清单

### 数据库验证
- [ ] `project_note_relation` 表已创建
- [ ] 唯一索引 `uk_project_note` 已创建
- [ ] 普通索引 `idx_project`, `idx_note`, `idx_note_type` 已创建
- [ ] 默认值正确设置（`arch_flag='1'`）

### 后端验证
- [ ] 项目编译成功，无错误
- [ ] 服务启动成功
- [ ] 日志中无 MyBatis 映射错误
- [ ] API 端点可访问

### 功能验证
- [ ] 能成功添加笔记到项目
- [ ] 能查询项目的关联笔记
- [ ] 能删除笔记关联
- [ ] 重复添加时返回正确错误提示
- [ ] 删除不存在的关联时正确处理

## 📋 新增文件清单

### 数据库脚本
- ✅ `backed/create_project_note_relation_table.sql` - 建表脚本
- ✅ `backed/test_project_note_relation.sql` - 测试脚本

### Java源代码
- ✅ `backed/src/main/java/com/knowledge/entity/ProjectNoteRelation.java`
- ✅ `backed/src/main/java/com/knowledge/mapper/ProjectNoteRelationMapper.java`
- ✅ `backed/src/main/java/com/knowledge/service/ProjectNoteRelationService.java`
- ✅ `backed/src/main/resources/mapper/ProjectNoteRelationMapper.xml`
- ✅ `backed/src/main/java/com/knowledge/controller/ProjectController.java` (已修改)

### 文档
- ✅ `backed/项目笔记关联功能部署说明.md`
- ✅ `backed/项目笔记关联功能快速部署指南.md` (本文件)

## 🔍 API接口快速参考

### 1. 获取项目的笔记ID列表
```http
GET /project/{projectId}/notes
```

### 2. 添加笔记到项目
```http
POST /project/{projectId}/notes
Body: {"noteId": 5, "noteType": "framework"}
```

### 3. 从项目移除笔记
```http
DELETE /project/{projectId}/notes/{noteId}
```

### 4. 获取项目的笔记数量
```http
GET /project/{projectId}/notes/count
```

### 5. 获取笔记关联的项目ID列表
```http
GET /project/notes/{noteId}/projects
```

## 🐛 常见问题快速解决

### Q1: 表已存在错误
```sql
-- 删除旧表
DROP TABLE IF EXISTS project_note_relation;

-- 重新执行建表脚本
source backed/create_project_note_relation_table.sql;
```

### Q2: Mapper找不到
```bash
# 确认mapper配置
cat backed/src/main/resources/application.properties | grep mybatis.mapper

# 应该输出：
# mybatis.mapper-locations=classpath:mapper/*.xml

# 如果没有，添加这行配置
```

### Q3: API返回404
```bash
# 检查服务是否启动
curl http://localhost:8080/project/list

# 检查日志
tail -f logs/application.log
```

### Q4: 添加关联失败
```sql
-- 检查项目是否存在
SELECT * FROM project WHERE id = 1;

-- 检查笔记是否存在（根据实际表名）
-- SELECT * FROM fragment_note WHERE id = 5;

-- 检查是否已存在关联
SELECT * FROM project_note_relation 
WHERE project_id = 1 AND note_id = 5;
```

## 📊 性能测试

### 测试脚本
```sql
-- 插入1000条测试数据
DELIMITER //
CREATE PROCEDURE insert_test_data()
BEGIN
    DECLARE i INT DEFAULT 1;
    WHILE i <= 1000 DO
        INSERT INTO project_note_relation 
        (project_id, note_id, note_type, rec_creator, arch_flag)
        VALUES 
        (FLOOR(1 + RAND() * 10), i, 'framework', 'admin', '1');
        SET i = i + 1;
    END WHILE;
END//
DELIMITER ;

-- 执行存储过程
CALL insert_test_data();

-- 性能测试查询
SELECT BENCHMARK(1000, (
    SELECT COUNT(*) FROM project_note_relation WHERE project_id = 1
));
```

### 预期性能指标
- 单次查询 < 10ms
- 单次插入 < 50ms
- 1000条数据查询 < 100ms

## 🎯 前端集成

前端代码已经准备好，位于：
- `fronted/src/views/daily/ProjectRecord.vue` (已修改)

前端API调用示例：
```typescript
// 获取项目笔记
const response = await request.get(`/project/${projectId}/notes`)

// 添加笔记
const response = await request.post(`/project/${projectId}/notes`, {
  noteId: noteId
})

// 删除笔记
const response = await request.delete(`/project/${projectId}/notes/${noteId}`)
```

## 🎉 部署完成

部署完成后：
1. 前端"笔记调用"功能将正常工作
2. 可以为项目关联笔记
3. 支持搜索、筛选和查看笔记
4. 所有操作会持久化到数据库

## 📞 技术支持

如遇问题，请检查：
1. 数据库表是否正确创建
2. 后端服务是否正常启动
3. API接口是否可访问
4. 前端控制台是否有错误

查看详细文档：`backed/项目笔记关联功能部署说明.md`





















