<template>
  <section id="diplomatic-projects-dashboard" class="p-6 animate-fade-in">
    <!-- 页面头部 -->
    <div class="mb-8">
      <div class="flex items-center space-x-4 mb-4">
        <h1 class="text-3xl font-bold text-gray-900">外交行动</h1>
        <span class="bg-green-100 text-green-600 px-3 py-1 rounded-full text-sm font-medium">对外推广</span>
      </div>
      <p class="text-gray-600">已有成果对外分享、技术交流、利益互动的综合管理中心</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
      ...
      <div class="bg-white rounded-xl shadow-soft p-6">
        ...
        <p class="text-2xl font-bold text-blue-600">{{ diplomaticProjects.filter(p => p.status === 'active').length }} 个</p>
        <p class="text-sm text-gray-500 mt-2">正在推广的项目</p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-soft p-6">
        ...
        <p class="text-2xl font-bold text-green-600">{{ diplomaticProjects.filter(p => p.status === 'completed').length }} 个</p>
      ...
      <div class="bg-white rounded-xl shadow-soft p-6">
        ...
        <p class="text-2xl font-bold text-purple-600">{{ diplomaticProjects.filter(p => p.status === 'planning').length }} 个</p>
      ...
      <div class="bg-white rounded-xl shadow-soft p-6">
        ...
        <p class="text-2xl font-bold text-orange-600">{{ diplomaticTasks.length }} 个</p>
    ...
    <!-- 外交行动项目区域 -->
    <div class="space-y-6">
      <h3 class="text-xl font-semibold text-gray-900">外交行动项目</h3>
      
      <!-- 空状态 -->
      <div v-if="diplomaticProjects.length === 0" class="bg-white rounded-xl shadow-soft p-12 text-center">
        <i class="fas fa-folder-open text-6xl text-gray-300 mb-4"></i>
        <p class="text-gray-500 text-lg">暂无外交行动项目</p>
        <p class="text-gray-400 text-sm mt-2">开始创建你的第一个推广项目吧！</p>
      </div>

      <!-- 项目列表 -->
      <div v-for="project in diplomaticProjects" :key="project.id" class="bg-white rounded-xl shadow-soft p-6">
        <div class="flex items-center justify-between mb-4">
        ...
        <!-- 项目任务 -->
        <div v-if="diplomaticTasks.filter(t => t.projectId === project.id).length > 0" class="space-y-3 mt-4">
          <h5 class="text-sm font-medium text-gray-700 mb-2">任务列表：</h5>
          <div v-for="task in diplomaticTasks.filter(t => t.projectId === project.id)" :key="task.id" 
               :class="{
...
const loadData = async () => {
  try {
    console.log('【外交行动】开始加载数据...')
    // 加载外交行动类别的项目
    const projectRes = await request.get('/project/category/diplomatic')
    if (projectRes.code === 200) {
      projects.value = projectRes.data || []
      console.log('【外交行动】加载项目数据:', projects.value.length, '个')
    }
    ...
    if (taskRes.code === 200) {
      ...
      console.log('【外交行动】加载任务数据:', tasks.value.length, '个')
    }
  } catch (error) {
    console.error('【外交行动】加载数据失败:', error)
  }
}
...
const diplomaticTasks = computed(() => {
  return tasks.value
})
