# 项目笔记关联功能部署说明

## 📋 概述

项目笔记关联功能允许为项目关联相关的笔记，方便在项目执行时快速调用相关知识和资料。

## 🗄️ 数据库部署

### 1. 执行SQL脚本

执行文件：`backed/create_project_note_relation_table.sql`

```sql
-- 创建项目笔记关联表
CREATE TABLE IF NOT EXISTS project_note_relation (
    id INT PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
    project_id INT NOT NULL COMMENT '项目ID',
    note_id INT NOT NULL COMMENT '笔记ID',
    note_type VARCHAR(50) NOT NULL COMMENT '笔记类型',
    rec_creator VARCHAR(50) COMMENT '创建人',
    rec_create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    rec_revisor VARCHAR(50) COMMENT '修改人',
    rec_revise_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    arch_flag CHAR(1) DEFAULT '1' COMMENT '归档标志',
    UNIQUE KEY uk_project_note (project_id, note_id),
    INDEX idx_project (project_id),
    INDEX idx_note (note_id),
    INDEX idx_note_type (note_type)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='项目笔记关联表';
```

### 2. 验证表创建

```sql
-- 查看表结构
DESC project_note_relation;

-- 查看索引
SHOW INDEX FROM project_note_relation;
```

## 📁 后端代码文件

### 新增文件列表

1. **实体类**
   - `backed/src/main/java/com/knowledge/entity/ProjectNoteRelation.java`

2. **Mapper接口**
   - `backed/src/main/java/com/knowledge/mapper/ProjectNoteRelationMapper.java`

3. **Mapper XML**
   - `backed/src/main/resources/mapper/ProjectNoteRelationMapper.xml`

4. **Service服务类**
   - `backed/src/main/java/com/knowledge/service/ProjectNoteRelationService.java`

5. **Controller更新**
   - `backed/src/main/java/com/knowledge/controller/ProjectController.java`（已修改）

## 🔌 API接口说明

### 1. 获取项目关联的笔记ID列表

```http
GET /project/{projectId}/notes
```

**响应示例：**
```json
{
    "code": 200,
    "message": "success",
    "data": [1, 5, 8, 12]
}
```

### 2. 添加笔记到项目

```http
POST /project/{projectId}/notes
Content-Type: application/json

{
    "noteId": 5,
    "noteType": "framework"
}
```

**响应示例：**
```json
{
    "code": 200,
    "message": "笔记已关联到项目",
    "data": null
}
```

**错误响应：**
```json
{
    "code": 500,
    "message": "笔记已经关联到该项目",
    "data": null
}
```

### 3. 从项目移除笔记

```http
DELETE /project/{projectId}/notes/{noteId}
```

**响应示例：**
```json
{
    "code": 200,
    "message": "已移除笔记关联",
    "data": null
}
```

### 4. 获取项目的笔记数量

```http
GET /project/{projectId}/notes/count
```

**响应示例：**
```json
{
    "code": 200,
    "message": "success",
    "data": 5
}
```

### 5. 获取笔记关联的项目ID列表

```http
GET /project/notes/{noteId}/projects
```

**响应示例：**
```json
{
    "code": 200,
    "message": "success",
    "data": [2, 7, 15]
}
```

## 🚀 部署步骤

### 1. 数据库部署

```bash
# 连接MySQL
mysql -u root -p

# 选择数据库
use knowledge_management;

# 执行SQL脚本
source /path/to/backed/create_project_note_relation_table.sql;

# 验证创建成功
DESC project_note_relation;
```

### 2. 后端代码部署

#### 方式一：完整重新编译（推荐）

```bash
cd backed

# 清理并重新编译
mvn clean package -DskipTests

# 停止旧服务
./stop.sh

# 启动新服务
./start.sh
```

#### 方式二：快速热部署（开发环境）

如果使用IDE（如IntelliJ IDEA）：
1. 直接运行主类 `KnowledgeManagementApplication`
2. IDE会自动编译新增的类

### 3. 验证部署

#### 测试API接口

```bash
# 1. 获取项目1的关联笔记（应该返回空数组）
curl http://localhost:8080/project/1/notes

# 2. 添加笔记5到项目1
curl -X POST http://localhost:8080/project/1/notes \
  -H "Content-Type: application/json" \
  -d '{"noteId": 5, "noteType": "framework"}'

# 3. 再次获取项目1的关联笔记（应该返回[5]）
curl http://localhost:8080/project/1/notes

# 4. 获取笔记数量
curl http://localhost:8080/project/1/notes/count

# 5. 移除关联
curl -X DELETE http://localhost:8080/project/1/notes/5
```

#### 查看数据库

```sql
-- 查看关联记录
SELECT * FROM project_note_relation;

-- 查看某个项目的关联
SELECT * FROM project_note_relation WHERE project_id = 1;

-- 查看某个笔记的关联
SELECT * FROM project_note_relation WHERE note_id = 5;
```

## 📊 核心功能说明

### 1. ProjectNoteRelationService 核心方法

| 方法名 | 功能 | 返回值 |
|--------|------|--------|
| `getNoteIdsByProjectId` | 获取项目的笔记ID列表 | `List<Integer>` |
| `getProjectIdsByNoteId` | 获取笔记的项目ID列表 | `List<Integer>` |
| `addProjectNoteRelation` | 添加项目笔记关联 | `boolean` |
| `removeProjectNoteRelation` | 删除指定关联 | `boolean` |
| `removeAllNotesByProjectId` | 删除项目的所有笔记关联 | `int` |
| `removeAllProjectsByNoteId` | 删除笔记的所有项目关联 | `int` |
| `countNotesByProjectId` | 获取项目的笔记数量 | `int` |
| `countProjectsByNoteId` | 获取笔记的项目数量 | `int` |
| `isRelated` | 检查是否已关联 | `boolean` |

### 2. 数据一致性保证

- ✅ 使用 `UNIQUE KEY` 防止重复关联
- ✅ 使用 `@Transactional` 保证事务一致性
- ✅ 添加前检查是否已存在关联
- ✅ 支持级联删除（可选）

### 3. 性能优化

- ✅ 在 `project_id` 和 `note_id` 上建立索引
- ✅ 使用 `arch_flag` 软删除机制
- ✅ 查询时自动过滤已归档记录

## 🔧 常见问题

### 1. 表创建失败

**问题**：执行SQL时提示表已存在
```
ERROR 1050 (42S01): Table 'project_note_relation' already exists
```

**解决**：
```sql
-- 删除旧表（注意：会丢失数据）
DROP TABLE IF EXISTS project_note_relation;

-- 重新执行创建脚本
source create_project_note_relation_table.sql;
```

### 2. MyBatis映射找不到

**问题**：启动时报错找不到Mapper
```
org.apache.ibatis.binding.BindingException: Invalid bound statement
```

**解决**：
1. 检查 `application.properties` 中的mapper配置：
```properties
mybatis.mapper-locations=classpath:mapper/*.xml
```

2. 确保XML文件在 `src/main/resources/mapper/` 目录下

3. 重新编译项目：
```bash
mvn clean compile
```

### 3. 添加关联时报错

**问题**：添加时提示 "笔记已经关联到该项目"

**原因**：数据库中已存在该关联记录

**解决**：
```sql
-- 查看是否已存在
SELECT * FROM project_note_relation 
WHERE project_id = ? AND note_id = ?;

-- 如需删除
DELETE FROM project_note_relation 
WHERE project_id = ? AND note_id = ?;
```

## 📈 后续优化建议

### 1. 批量操作支持

```java
// 添加到Service中
public boolean addNotesToProject(Integer projectId, List<Integer> noteIds, String username);
public int removeNotesFromProject(Integer projectId, List<Integer> noteIds);
```

### 2. 关联关系元数据

可扩展字段：
- `relation_desc` - 关联说明
- `importance` - 重要程度（1-5星）
- `tags` - 自定义标签
- `order_num` - 排序序号

### 3. 关联统计分析

```java
// 统计最常被关联的笔记
List<Map<String, Object>> getMostReferencedNotes(int limit);

// 统计项目的笔记类型分布
Map<String, Integer> getNoteTypeDistribution(Integer projectId);
```

### 4. 级联删除策略

考虑在删除项目或笔记时，是否自动清理关联关系：

```java
// 在ProjectService中添加
@Transactional
public boolean deleteProject(Integer projectId) {
    // 先删除关联
    projectNoteRelationService.removeAllNotesByProjectId(projectId);
    // 再删除项目
    return projectMapper.deleteByPrimaryKey(projectId) > 0;
}
```

## ✅ 验收标准

### 功能验收
- [ ] 能成功添加笔记到项目
- [ ] 能查询项目的所有关联笔记
- [ ] 能移除笔记关联
- [ ] 重复添加时正确提示
- [ ] 删除不存在的关联时正确处理

### 性能验收
- [ ] 查询100个项目的关联笔记 < 1s
- [ ] 添加单个关联 < 100ms
- [ ] 删除单个关联 < 100ms

### 数据验收
- [ ] 关联记录正确保存到数据库
- [ ] rec_creator 正确记录当前用户
- [ ] rec_create_time 自动设置
- [ ] arch_flag 默认为'1'

## 🎉 完成

部署完成后，前端的"笔记调用"功能将能正常工作，实现项目-笔记的关联管理！
















